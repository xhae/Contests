...
int main()
{
	...
	for(int kase = 1; kase <= T; kase++) {
		...
		int team[250][2], guest[250][2];
		for(int i = 0; i < h; i++) scanf("%d %d", team[i], team[i] + 1);
		for(int i = 0; i < g; i++) scanf("%d %d", guest[i], guest[i] + 1);

		memset(cap, 0, sizeof(cap));

		int s = h + g, e = s + 1;
		for(int i = 0; i < h; i++) cap[s][i] = 1;
		for(int i = 0; i < g; i++) cap[h + i][e] = 1;
		for(int i = 0; i < h; i++)
			for(int j = 0; j < g; j++) {
				int curCost = abs(team[i][0] - guest[j][0]) + abs(team[i][1] - guest[j][1]) + abs(guest[j][0] - K) + abs(guest[j][1] - K);
				if(curCost > c) continue;
				cap[i][h + j] = 1;
			}

		int fcost;
		int flow = mcmf3( e + 1, s, e, fcost );
		printf("Case %d: %d\n", kase, flow);
	}
