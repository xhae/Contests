...
struct Frac {
	BigInt a, b;
	Frac() {}
	Frac(BigInt _a, BigInt _b) {
		if(_a == BigInt(0)) {
			a = 0;
			b = 1;
		} else {
			bool flag = false;
			if(_a < 0) { _a *= -1; flag = !flag; }
			if(_b < 0) { _b *= -1; flag = !flag; }
			BigInt v = getGCD(_a, _b);
			a = _a / v;
			b = _b / v;
			if(flag) a *= -1;
		}
	...
	Frac operator * (Frac tar) {
		return Frac(a * tar.a, b * tar.b);
	}
	Frac operator / (Frac tar) {
		return (*this) * Frac(tar.b, tar.a);
	}
	Frac operator + (Frac tar) {
		return Frac(a * tar.b + tar.a * b, b * tar.b);
	}
	void print() {
		a.print();
		printf("/");
...
void printMat() {
	for(int i = 0; i < n * n; i++) {
		for(int j = 0; j < n * n; j++) mat[i][j].print();
		printf("\n");
	}
}
			...
			for(int j = 0; j < n; j++)
				for(int ni = 0; ni < n; ni++)
					for(int nj = 0; nj < n; nj++) {
						Frac &tar = mat[i * n + j][ni * n + nj];
						tar = Frac(0, 1);
						if(i == ni and j == nj) {
							bool yend = (i == 0 or i == n - 1);
							bool xend = (j == 0 or j == n - 1);
							if(yend and xend) tar = Frac(BigInt(3), BigInt(1));
							else if(yend or xend) tar = Frac(BigInt(5), BigInt(1));
							else tar = Frac(BigInt(8), BigInt(1));
						} else {
							for(int dy = -1; dy <= 1; dy++)
								for(int dx = -1; dx <= 1; dx++)
									if(i + dy == ni and j + dx == nj) {
										tar = Frac(BigInt(-1), BigInt(1));
									}
						}
					}
...
//			printf("\n");
			for(int j = i + 1; j < n * n - 1; j++) {
				Frac coeff = mat[j][i] / mat[i][i] * Frac(BigInt(-1), BigInt(1));
				for(int k = 0; k < n * n; k++) 
...
//			printMat();
		...
		Frac ans(1, 1);
		for(int i = 0; i < n * n - 1; i++) ans = ans * mat[i][i];
		ans.a.print();		
		printf("\n");
	}
